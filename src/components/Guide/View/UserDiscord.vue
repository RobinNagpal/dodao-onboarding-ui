...
const props = defineProps({
  ...
  discordResponse: { type: Object, required: true }
});

const url = computed(() => {
  const guideParams = `guideType=${props.guideType}&guideUuid=${props.guideUuid}`;
  const stepParams = `stepUuid=${props.stepUuid}&stepOrder=${props.stepOrder}&itemUuid=${props.userDiscord.uuid}`;
  const params = new URLSearchParams({
    scope: 'identify email guilds.join',
    client_id: discordClientId,
    response_type: 'token',
    state: `spaceId=${props.spaceId}&${guideParams}&${stepParams}&target=guideView`,
  ...
  <div class="w-full flex justify-center">
    <div class="my-4">
      <a :href="url" class="text-white discord-btn inline-flex button px-[24px]">
        <div class="h-[32px] w-full rounded-full mr-2 items-center justify-center">
          <div v-if="!discordResponse" class="text-white ml-2">
          ...
          <div v-else class="text-white flex align-center">
            <img
              class="discord-image rounded-full"
              :src="`https://cdn.discordapp.com/avatars/${discordResponse.id}/${discordResponse.avatar}.png`"
            />
            <div class="ml-2">
              {{ discordResponse.username }}
            </div>
          </div>
        </div>
      </a>
