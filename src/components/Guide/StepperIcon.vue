...
const props = defineProps({
  step: { type: Object as PropType<GuideStep>, required: true }
});
const hasQuestions = props.step.stepItems.some(item => isQuestion(item));
const hasInputs = props.step.stepItems.some(item => isUserInput(item));
</script>
<template>
  <svg
    v-if="hasQuestions"
